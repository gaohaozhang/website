<?php if (!defined('THINK_PATH')) exit();?><!-- layout::Public:head::60 -->
<div class="main mainbg clearfix">
  <div class=leftM>
    <div class=ban1 style="width:220px;">
      <h2><a href="__APP__/JiajiaoInfo/qing_edu"><span>请家教</span></a></h2>
      <ul>
        <li><a href="__APP__/InfoDown/downloadlist">资源下载</a>
        <li><a href="__APP__/SpinfoDetail/videolist">教学视频</a>
        <li><a href="__APP__/TeachInfo/tehlist">挑选合适的教员</a></li>
      </ul>
    </div>
    <div class=ban2 style="width:220px;">
      <h2><a href="__APP__/TeachInfo/zuo_edu"><span>做家教</span></a></h2>
      <ul>
        <li><a href="__APP__/JiajiaoInfo/edu_info">家教信息</a>
        <li><a href="__APP__/InfoDetail/edu_news">助教咨询</a>
        <li><a href="__APP__/InfoDetail/infolist/ttedu_id/25">好书推荐</a>
        </li>
      </ul>
    </div>
    <div class=ban3 style="width:220px;">
      <em>联系我们</em>
      <p>
        <span>咨询电话：<?php echo ($webinfo["ttedu_tel"]); ?></span><br>
        <span >家教工作时间</span><span > 9：00-20：00 </span><a href="http://wpa.qq.com/msgrd?V=1&amp;Uin=<?php echo ($webinfo["ttedu_qq"]); ?>&amp;Site=wh.eduease.com&amp;Menu=yes" target=blank><img alt=点击这里给我发消息 src="Public/images/qq.gif"></a><br>
        <span >节假日有人值班！</span>
      </p>
    </div>
    <div class=hackbox>
    </div>
    <div class=titseach>
      <script language=javascript src="Public/js/index.js"></script>
      <div class="seacbg2 clearfix">
        <div class=seacbg2long1>
          <em><a class="fr" href="__APP__/index/searchteach">-点击更多-</a>
            <h3><i class="fa fa-table"></i><a href="__APP__/index/searchteach">高校分类</a></h3>
          </em>
          <p>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/复旦大学">复旦大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海交通大学">上海交通大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/同济大学">同济大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/华东师范大学">华东师范大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/华东理工大学">华东理工大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海财经大学">上海财经大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海海事大学">上海海事大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/东华大学">东华大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海理工大学">上海理工大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海师范大学">上海师范大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海中医药大学">上海中医药大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/华东政法大学">华东政法大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海海洋大学">上海海洋大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海工程技术大学">上海工程技术大学</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海商学院">上海商学院</a>
            <a href="__APP__/index/searchteach/ttedu_biyexuexiao/上海电机学院">上海电机学院</a>
          </p>
        </div>
        <div class=seacbg2long>
          <em><a class="fr" href="__APP__/index/searchteach">-点击更多-</a>
            <h3><i class="fa fa-th-list"></i><a href="__APP__/index/searchteach">科目分类</a></h3>
          </em>
          <p>
            <a href="__APP__/index/searchteach/ttedu_renjiaoxueke/数学">数学</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/英语">英语</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/语文">语文</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/物理">物理</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/化学">化学</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/生物">生物</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/地理">地理</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/历史">历史</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/政治">政治</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/作文">作文</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/奥数">奥数</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/钢琴">钢琴</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/电子琴">电子琴</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/古筝">古筝</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/竹笛">竹笛</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/美术">美术</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/日语">日语</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/德语">德语</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/法语">法语</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/韩语">韩语</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/俄语">俄语</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/雅思">雅思</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/托福">托福</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/计算机">计算机</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/英语口语">英语口语</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/幼儿">幼儿</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/小学">小学</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/初中">初中</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/高中">高中</a>
          </p>
        </div>
        <div id=diqv>
          <em><a class="fr" href="__APP__/index/searchteach">-点击更多-</a>
            <h3><i class="fa fa-th"></i><a href="__APP__/index/searchteach">区域分类</a></h3>
          </em>
          <p>
            <a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/黄浦区">黄浦区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/徐汇区">徐汇区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/长宁区">长宁区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/静安区">静安区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/普陀区">普陀区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/虹口区">虹口区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/杨浦区">杨浦区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/闵行区">闵行区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/宝山区">宝山区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/嘉定区">嘉定区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/浦东新区">浦东新区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/金山区">金山区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/松江区">松江区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/青浦区">青浦区</a><a href="__APP__/index/searchteach/ttedu_jiajiaodiqu/奉贤区">奉贤区</a>
          </p>
        </div>
      </div>
    </div>
    <!--[if !IE]>右部两栏<![endif]-->
    <div>
      <div class=leftM1 style="width:490px;">
        <div class=tit01>
          <div class=tit01TB1>
            <a href="__APP__/index/searchteach">-更多-</a>
            <p>
              <a  href="__APP__/index/searchteach/ttedu_renjiaoxueke/语文">语文家教</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/数学">数学家教</a>
            </p>
            <h2><i class="fa fa-pencil"></i> 专业教师库</h2>
          </div>
          <ol class=line02>
            <li class=lin02bg><em>姓名</em>
              <address>性别</address><i>执教学科</i><b>身份</b><u>专业</u><em>认证时间</em></li>
            <?php if(is_array($zhuanyejiaoshi)): $i = 0; $__LIST__ = $zhuanyejiaoshi;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><em><a href="__APP__/TeachInfo/tehdetail/id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo (msubstr($vo["ttedu_chenfu"],0,1,'utf-8',false)); ?>老师</a></em>
                <address>&nbsp;<?php echo ($vo["ttedu_sex"]); ?> </address>
                <i>&nbsp;<?php echo ($vo["ttedu_renjiaoxueke"]); ?></i>
                <b>&nbsp; <?php echo ($vo["ttedu_mqshengfei"]); ?></b>
                <u>&nbsp;<?php echo ($vo["ttedu_zhuanye"]); ?></u>
                <em class=yel>&nbsp;<?php echo (date('Y-m-d',$vo["ttedu_zhucedate"])); ?></em></li><?php endforeach; endif; else: echo "" ;endif; ?>
          </ol>
        </div>
      </div>
      <div class=rightM1>
        <div class=tit01T2>
          <a href="__APP__/index/searchteach">-更多-</a>
          <h2> <i class="fa fa-star"></i> 星级教员 </h2>
        </div>
        <ul class=tit01T2B>
          <li><a id=t_1 class=tit01T2Bc onmouseover=x:Hover(1); href="__APP__/index/searchteach/ttedu_renjiaoxueke/语文">语文家教</a></li>
          <li><a id=t_2 onmouseover=i:Hover(2); href="__APP__/index/searchteach/ttedu_renjiaoxueke/数学">数学家教</a></li>
          <li><a id=t_3 onmouseover=i:Hover(3); href="__APP__/index/searchteach/ttedu_renjiaoxueke/英语">英语家教</a></li>
        </ul>
        <ol style="DISPLAY: block" id=tbc1 class="line0r2 cle">
          <?php if(is_array($yuwenjiaoshi)): $i = 0; $__LIST__ = $yuwenjiaoshi;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li class=line0r2bg><a href="__APP__/TeachInfo/tehdetail/id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo (msubstr($vo["ttedu_chenfu"],0,1,'utf-8',false)); ?>老师 <span class=green>详细介绍</span>
              <?php if(($vo["ttedu_shifoushengji"])  >  "0"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "1"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "2"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "3"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "4"): ?><img src="Public/images/d18.gif"><?php endif; ?>
            </a></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ol>
        <ol style="DISPLAY: none" id=tbc2 class="line0r2 cle">
          <?php if(is_array($shuxuejiaoshi)): $i = 0; $__LIST__ = $shuxuejiaoshi;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li class=line0r2bg><a href="__APP__/TeachInfo/tehdetail/id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo (msubstr($vo["ttedu_chenfu"],0,1,'utf-8',false)); ?>老师 <span class=green>详细介绍</span>
              <?php if(($vo["ttedu_shifoushengji"])  >  "0"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "1"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "2"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "3"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "4"): ?><img src="Public/images/d18.gif"><?php endif; ?>
            </a></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ol>
        <ol style="DISPLAY: none" id=tbc3 class="line0r2 cle">
          <?php if(is_array($yingyujiaoshi)): $i = 0; $__LIST__ = $yingyujiaoshi;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><a href="__APP__/TeachInfo/tehdetail/id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo (msubstr($vo["ttedu_chenfu"],0,1,'utf-8',false)); ?>老师 <span class=green>详细介绍</span>
              <?php if(($vo["ttedu_shifoushengji"])  >  "0"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "1"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "2"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "3"): ?><img src="Public/images/d18.gif"><?php endif; ?>
              <?php if(($vo["ttedu_shifoushengji"])  >  "4"): ?><img src="Public/images/d18.gif"><?php endif; ?>
            </a></li><?php endforeach; endif; else: echo "" ;endif; ?>
        </ol>
      </div>
    </div>
  </div>
  <!--[if !IE]>上部左区结束，右区开始<![endif]-->
  <div class=rightM>
    <div class=loginbg>
      <div class=loginT>
        <i class="fa fa-user-circle-o"></i>
        <div class="tit01T fr">
          <h2><span style="color:#ffffff">登录</span>&nbsp;&nbsp;&nbsp;&nbsp;</h2>
        </div>
      </div>
      <div class=loginT1>
        <form method=post name=adForm action="__APP__/Login/login/types/1">
          <p>
            <label for=name>用户名：</label><input id=username class=loginT1input type=text name="username">
          </p>
          <p>
            <label for=pass>密&nbsp;&nbsp;&nbsp;码：</label><input id=pass class=loginT1input type=password name="password">
          </p>
          <p>
            <label for=yz>身&nbsp;&nbsp;&nbsp;份：</label>
            <label>
              <span style="margin-right:30px;"><input type="radio" value="1" name="usertype" checked="checked"> 老师</span>
              <span><input type="radio" value="2" name="usertype"> 学生</span>
            </label>
          <p class=ac>
            <input style="CURSOR: pointer" id=button2 class=butblue value=" 登 录 " type=submit name=button2>&nbsp;&nbsp; <input style="CURSOR: pointer" id=button2 class=butblue value=" 重 置 " type=reset name=button2>
          </p>
        </form>
        <em><i class="fa fa-mortar-board"></i><a href="__APP__/HuiyuanInfo/xuesheng_zhuce">学生注册</a>&nbsp;&nbsp;&nbsp;
          <i class="fa fa-paint-brush"></i><a href="__APP__/TeachInfo/zuo_edu" target=_blank>老师注册</a>
        </em>
      </div>
    </div>
    <!--[if !IE]>登陆结束<![endif]-->
    <div class=tit01T1>
      <a href="__APP__/SpinfoDetail/videolist" target=_blank>-更多-</a>
      <h2> <i class="fa fa-play-circle-o"></i> 教学视频</h2>
    </div>
    <div class=focusPic>
      <embed src=" http://player.youku.com/player.php/sid/XNDY0MjA4/v.swf" quality="high" width="100%" height="226" align="middle" allowscriptaccess="sameDomain" type="application/x-shockwave-flash"></embed>
    </div>
    <ol class=line01>
      <?php if(is_array($jiaoxueshipin)): $i = 0; $__LIST__ = $jiaoxueshipin;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><a href="__APP__/SpinfoDetail/videodetail/ttedu_id/<?php echo ($vo["ttedu_spbtype_id"]); ?>/jishu/0" target=_blank><?php echo ($vo["ttedu_sptitle"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
    </ol>
  </div>
  <!--[if !IE]>上部结束，中部左区开始<![endif]-->
  <div class="main ie6main">
    <div class=leftM>
      <div class=leftM1 style="width:480px;">
        <div class=tit01>
          <div class=tit01TB1>
            <a href="__APP__/index/searchteach">-更多-</a>
            <p>
              <a href="__APP__/index/searchteach/ttedu_renjiaoxueke/语文">语文家教</a><a href="__APP__/index/searchteach/ttedu_renjiaoxueke/数学">数学家教</a>
            </p>
            <h2><i class="fa fa-pencil-square-o"></i> 大学生教师库</h2>
          </div>
          <ol class=line02>
            <li class=lin02bg><em>姓名</em>
              <address>性别</address><i>学历</i><u>专业</u><b>高校</b><em>认证时间</em></li>
            <?php if(is_array($daxueshengjiaoshi)): $i = 0; $__LIST__ = $daxueshengjiaoshi;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><em><a href="__APP__/TeachInfo/tehdetail/id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo (msubstr($vo["ttedu_chenfu"],0,1,'utf-8',false)); ?>老师</a></em>
                <address>&nbsp;<?php echo ($vo["ttedu_sex"]); ?></address>
                <i>&nbsp;<?php echo ($vo["ttedu_mqxueli"]); ?></i>
                <u>&nbsp;<?php echo ($vo["ttedu_zhuanye"]); ?></u>
                <b>&nbsp;<?php echo ($vo["ttedu_biyexuexiao"]); ?></b>
                <em class=yel>&nbsp;<?php echo (date('Y-m-d',$vo["ttedu_zhucedate"])); ?></em></li><?php endforeach; endif; else: echo "" ;endif; ?>
          </ol>
        </div>
        <div class=tit01>
          <div class=tit01TB1>
            <a href="__APP__/JiajiaoInfo/edu_info">-更多-</a>
            <h2><i class="fa fa-navicon"></i> 家教信息</h2>
          </div>
          <ol class=line02>
            <li class=lin02bg><u>求教学科</u>
              <address>性别</address><i>年龄</i><u>授课区域</u>
              <b>要求</b><em>发布时间</em></li>
            <?php if(is_array($jiajiaoxinxi)): $i = 0; $__LIST__ = $jiajiaoxinxi;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><u><a title="<?php echo ($vo["ttedu_nianjikemu"]); ?>" href="__APP__/JiajiaoInfo/edu_infodetail/id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo ($vo["ttedu_nianjikemu"]); ?> </a></u>
                <address>&nbsp;<?php echo ($vo["ttedu_sex"]); ?></address>
                <i>&nbsp;<?php echo ($vo["ttedu_old"]); ?></i>
                <u>&nbsp;<?php echo ($vo["ttedu_shoukediqu"]); ?></u>
                <b>&nbsp;<?php echo ($vo["ttedu_laoshiyaoqiu"]); ?></b>
                <em class=yel>&nbsp;<?php echo (date('Y-m-d',$vo["ttedu_addtime"])); ?></em></li><?php endforeach; endif; else: echo "" ;endif; ?>
          </ol>
        </div>
      </div>
      <div class=rightM1>
        <div class=tit01>
          <div class=tit01T2>
            <a href="__APP__/InfoDown/downloadlist" target=_blank>-更多-</a>
            <h2> <i class="fa fa-sort-amount-asc"></i> 教学资料下载</h2>
          </div>
          <ol class=line00>
            <?php if(is_array($ziliaoxiazai)): $i = 0; $__LIST__ = $ziliaoxiazai;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><a href="__APP__/InfoDown/downloaddetail/ttedu_id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo ($vo["ttedu_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
          </ol>
        </div>
        <div class=tit01>
          <div class=tit01T2>
            <a href="__APP__/InfoDetail/infolist/ttedu_id/25" target=_blank>-更多-</a>
            <h2> <i class="fa fa-thumbs-o-up"></i> 好书推荐</h2>
          </div>
          <ol class=line00>
            <?php if(is_array($haoshutuijian)): $i = 0; $__LIST__ = $haoshutuijian;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><a href="__APP__/InfoDetail/infodetail/ttedu_id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo ($vo["ttedu_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
          </ol>
        </div>
      </div>
    </div>
    <!--[if !IE]>中区右部结束，左区开始<![endif]-->
    <div class="rightM rBboder">
      <div class=tit01T1>
        <a href="__APP__/InfoDetail/infolist/ttedu_id/23" target=_blank>-更多-</a>
        <h2><i class="fa fa-superscript"></i> 常见问题</h2>
      </div>
      <ol class=line01>
        <?php if(is_array($changjianwenti)): $i = 0; $__LIST__ = $changjianwenti;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><a href="__APP__/InfoDetail/infodetail/ttedu_id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo ($vo["ttedu_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ol>
      <div class=tit01T1>
        <a href="__APP__/InfoDetail/infolist/ttedu_id/24" target=_blank>-更多-</a>
        <h2><i class="fa fa-support"></i> 心灵沟通</h2>
      </div>
      <ol class=line01>
        <?php if(is_array($xinlingoutong)): $i = 0; $__LIST__ = $xinlingoutong;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><a href="__APP__/InfoDetail/infodetail/ttedu_id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo ($vo["ttedu_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ol>
      <div class=tit01T1>
        <a href="__APP__/InfoDetail/infolist/ttedu_id/26" target=_blank>-更多-</a>
        <h2><i class="fa fa-train"></i> 家教之路</h2>
      </div>
      <ol class=line01>
        <?php if(is_array($jiajiaozhilu)): $i = 0; $__LIST__ = $jiajiaozhilu;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><li><a href="__APP__/InfoDetail/infodetail/ttedu_id/<?php echo ($vo["ttedu_id"]); ?>" target=_blank><?php echo ($vo["ttedu_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ol>
    </div>
  </div>
</div>
<!--下部资讯-->
<!--下部资讯结束-->
<div class=hackbox>
</div>
<!--主区结束，底区开始-->
<!--友情链接-->
<div class=ljmain>
    <div class=ljnr>
    </div>
    <div class=ljnr1>
    </div>
    <div class=ljnr>
        <span class=red>[友情链接]：</span>
        <?php if(is_array($set_youqing)): $i = 0; $__LIST__ = $set_youqing;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): ++$i;$mod = ($i % 2 )?><a href="<?php echo ($vo["ttedu_website"]); ?>" title="<?php echo ($vo["ttedu_webname"]); ?>" target="_blank"><?php echo ($vo["ttedu_webname"]); ?></a><?php endforeach; endif; else: echo "" ;endif; ?>
    </div>
</div>
<!--主区结束，底区开始-->
<div class=hackbox>
</div>
<div class=footer>
    <p>
        Copyright © 2017 Tencent. All Rights Reserved.
    </p>
    <p>
        上海市某某某某有限公司 版权所有
    </p>
</div>
<script type=text/javascript>
    var gaJsHost = (("https:" == document.location.protocol) ? "https://ssl." : "http://www.");
    document.write(unescape("%3Cscript src='" + gaJsHost + "google-analytics.com/ga.js' type='text/javascript'%3E%3C/script%3E"));
</script>
<script type=text/javascript>
    try {
        var pageTracker = _gat._getTracker("UA-770217-10");
        pageTracker._trackPageview();
    } catch(err) {}</script>
</body>
</html>